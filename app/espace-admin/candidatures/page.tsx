...
export default function EspaceAdminCandidaturesPage() {
  ...
  const loadApplications = useCallback(async (token: string) => {
    ...
    const result = await requestApi<AdminApplication[]>(
      "/api/admin/applications?limit=200&sort=newest",
      {
        method: "GET",
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      if (!session) {
        router.replace("/login?next=/espace-admin/candidatures");
        return;
      }
  ...
  const handleOpenCv = async (application: AdminApplication) => {
    if (!accessToken) {
      return;
    }

    if (!application.cv_path) {
      setActionFeedback("Aucun CV disponible pour cette candidature.");
      return;
    }

    setActionFeedback(null);
    setBusyCvId(application.id);

    const result = await requestApi<{ download_url: string }>(
      `/api/admin/applications/${encodeURIComponent(application.id)}/cv`,
      {
        method: "GET",
  ...
  const handleOpenMotivation = async (application: AdminApplication) => {
    if (!accessToken) {
      return;
    }

    if (!application.motivationLetterPath) {
      setActionFeedback("Aucun fichier de lettre de motivation disponible.");
      return;
    }

    setActionFeedback(null);
    setBusyMotivationId(application.id);

    const result = await requestApi<{ download_url: string }>(
      `/api/admin/applications/${encodeURIComponent(application.id)}/motivation-letter`,
      {
        method: "GET",
        headers: toAuthHeader(accessToken),
      }
                  ...
                  >
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          void handleOpenMotivation(application);
                        }}
